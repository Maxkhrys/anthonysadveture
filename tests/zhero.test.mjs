...
export default async function (page, R) {
  ...
  await page.evaluate(async () => {
    ...
    const pick = (base, r = 2, unique) => { for (let i = 0; i < 400; i++) { const it = I.genItem({ developer:true, level: 16, rarity: r, slot: null }); if (it.base === base) return it; } const it = I.genItem({ developer:true, level: 16, rarity: r }); it.base = base; return it; };
    mk('samurai', 58.5, 67, {}); mk('archer', 59.5, 67, {}); mk('witch', 60.5, 67, {});
    const L = I.LEGENDARIES;
    const leg = id => { for (let i = 0; i < 3000; i++) { const it = I.genItem({ developer:true, level: 16, rarity: 4 }); if (it.unique === id) return it; } return null; };
