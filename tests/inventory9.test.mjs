...
export default async function(page,R){
 ...
 await page.evaluate(()=>{const g=window.__game,I=window.__items;g.inv.bag=['heavensdivide','azureedge','moonfeather','verdanteclipse','fateweaver','phoenixfeather','worldseed','thornwood','umbraltome','dawnbringer','travelantern','wayfarersatchel'].map(id=>I.makeNamed(id,16));g.inv.bag.push(...Array.from({length:12},(_,i)=>I.genItem({ developer:true,level:16,slot:i%2?'armor':'ring',rarity:i%4})));for(const slot of ['helm','armor','arms','legs','boots','charm'])g.inv.equip[slot]=I.genItem({ developer:true,level:16,slot,rarity:3});g.inv.equip.weapon=I.makeNamed('verdanteclipse',16);g.recalc();g.inv.hp=g.inv.maxHp;g.inv.coins=1250;g.ui.navigate('bag');});
