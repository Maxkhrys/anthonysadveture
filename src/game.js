...
export class Game {
  ...
  atmosphere(dt) {
    ...
    if (a && a.glasshouse) {
      ...
      u.grade.value.set(1.04, 1.04, 0.96); u.desat.value = 0; u.cloudAmt.value = 0; u.beamAmt.value = 0.08; u.mistAmt.value = 0;
    ...
    // sunbeams: faint at noon, long and golden at dawn and dusk; morning mist burns off by mid-morning
    const WF = this.pr.worldFx || {};
    u.beamAmt.value = (WF.beams || 0) * Math.min(1, L * 1.6) * (0.5 + warm * 2.2) * (1 - r * 0.8);
    u.beamCol.value.setRGB(1, 0.93 - warm * 0.18, 0.78 - warm * 0.3);
    const morning = Math.max(0, 1 - Math.abs(day - 0.25) / 0.09);
    u.mistAmt.value = (WF.mist || 0) * Math.max(morning, r * 0.35);
    u.mistCol.value.setRGB(0.95 - N * 0.3, 0.93 - N * 0.25, 0.88 - N * 0.1);
