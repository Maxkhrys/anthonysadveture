...
import { geo, MAT } from './models.js';
import { grassBlock } from './hero_voxel.js';
...
// A grass block on stone (the concept sheet's plinth) and a soft contact shadow.
function plinth() {
  const g = new THREE.Group();
  const m = new THREE.Mesh(geo(grassBlock()), MAT); m.receiveShadow = true; m.layers.enable(1); g.add(m);
  const sh = new THREE.Mesh(new THREE.CircleGeometry(.24, 20).rotateX(-Math.PI/2), new THREE.MeshBasicMaterial({color:0x1a1420,transparent:true,opacity:.3,depthWrite:false}));
  sh.position.y = .004; g.add(sh);
  return g;
}
export class HeroPortrait {
 ...
 resize(){if(!this.pixel||!this.host)return;const r=this.host.getBoundingClientRect();if(r.width<1||r.height<1)return;const texel=(this.game?.pr.unitsPerPx||12/330)/PORTRAIT_DETAIL;
  ...
  const height=Math.max(1.72,1.3*r.height/r.width)*this.zoom;this.pixel.forceScale=Math.max(1,r.height*texel/height);this.pixel.resize();this.pixel.setViewHeight((this.pixel.rh-2)*texel);this.pixel.renderer.domElement.dataset.texelDensity=String(texel);
 }
